...
- (IBAction)checkoutDidTap:(id)sender
{
    ...
    order.SN = @"000001";
    order.totalPrice = [[NSDecimalNumber alloc] initWithFloat:10.00f];
    
    PaymentSuccessViewController *payment;
    payment = [[PaymentSuccessViewController alloc]
               initWithOrder:order
               andUser:self.user];
...
//        CartItem *cartItem = self.cart.items[0];
//        BOOL isExist = NO;
//        for (CartItem *cproduct in self.user.cart.products) {
//            if (cproduct.product.productID == cartItem.product.productID) {
//                cartItem.cartItemID = cproduct.cartItemID;
//                isExist = YES;
//            }
//        }
        
//        if (isExist)
//            [self.api requestCartUpdateWithUser:self.user
//                                           cart:self.user.cart
//                                    cartItem:cartItem];
        // FIXME create cart
//        else
//            [self.api requestCartCreateWithUser:[[Shop currentShop] currentUser]
